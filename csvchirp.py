...
offsetdirection = {"Minus": '-', "Plus": '+', "Simplex": '', "Split": ''}
offset = {'5.00 MHz': '5.00', '7.85 MHz': '7.85', '8.00 MHz': '8.00', '9.15 MHz': '9.15', '600 kHz': '0.600000', ' ': ''}
tone = {'Tone': 'Tone', 'None': ''}
  ...
  with open('chirp.csv', 'w', newline='') as csvoutput:
    ...
    for row in reader:
      outrow = {'Location': row['Channel Number'], 
        ...
        'Duplex': offsetdirection[row['Offset Direction']], 
        'Offset': offset[row['Offset Frequency']], 
        'Tone': tone[row['Tone Mode']], 
        'rToneFreq': row['CTCSS'], 
        'cToneFreq': row[''], 
        'DtcsCode': row['DCS'], 
        'DtcsPolarity': '',  # row[''], 
